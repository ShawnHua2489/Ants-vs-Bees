...
class Place(object):
    ...
    def __init__(self, name, exit=None): #place0 = Place('place_0') #place1 = Place('place_1', place0)
        ...
        # BEGIN Problem 2
        "*** YOUR CODE HERE ***" #P0(exit/S) P1(S/entrance)
        if self.exit != None:
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                assert self.ant.can_contain(insect) == True or insect.can_contain(self.ant) == True, 'Two ants in {0}'.format(self) #???Bad composition. ALso the "can contain"'s arguments are confusing'
                if self.ant.can_contain(insect) == True: #因为忘记加insect的argument导致debug20分钟...gg
                    self.ant.contain_ant(insect) # 'self.ant = self.ant.contain_ant(insect)' why wrong???
                elif insect.can_contain(self.ant) == True:
        ...
        insect.place = self #??? insect.place?
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            "*** YOUR CODE HERE ***"
            if isinstance(insect, QueenAnt) and insect.realqueen == True:
                self.ant = self.ant
                return #or it will keep checking, and finally go to the next if statement, which removes queen 
                       #here when it returns nothing, basically nothing happens 
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if self.place.ant == None:
            return False
        elif self.place.ant.blocks_path == False:
            return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        current_place = self.place #Why can we refer to place by self.place???
        while current_place != hive:
            if random_or_none(current_place.bees) != None:
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        ...
        current_place = self.place
        i = 0
        while current_place != hive and i < self.max_range:
            if random_or_none(current_place.bees) != None:
                return random_or_none(current_place.bees)
            i += 1
            current_place = current_place.entrance
...
class WallAnt(Ant):
    ...
    def __init__(self, armor=1):
        """Create an Ant with an ARMOR quantity."""
        Ant.__init__(self, 4) #??? __init__ default armor = 1 can be put like this
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        bee.reduce_armor(bee.armor) #RRR It's NOT self.bee 
    ...
    def action(self, colony):
        ...
        if self.digesting > 0:
            self.digesting -= 1
        elif random_or_none(self.place.bees) != None and self.digesting == 0:
...
class BodyguardAnt(Ant):
    ...
    def action(self, colony):
        ...
        "*** YOUR CODE HERE ***"
        if self.ant == None:
            Insect.action(self,colony) #(colony) why is it wrong???
        else: 
            self.ant.action(colony) #(colony)what's the point of "colony" being here???
        # END Problem 9
...
class TankAnt(BodyguardAnt):
    ...
    implemented = True   # Change to True to view in the GUI
    food_cost = 6
    container = True #this line is optional? 
    # END Problem 10
...
# END Problem 13
    ...
    def __init__(self):
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        ScubaThrower.__init__(self) #no need for "armor" argument?
        if QueenAnt.test == True: #there is no queen created before:
            QueenAnt.test = False
            self.realqueen = True
        elif QueenAnt.test == False:
    ...
    def action(self, colony):
        ...
        "*** YOUR CODE HERE ***"
        #doubled = []
        if self.realqueen == False: 
            Insect.reduce_armor(self, self.armor)
        elif self.realqueen == True:  
            ScubaThrower.action(self, colony)

            current_place = self.place.exit
            while current_place != None:
                #check there is ant in the place & its damage is not already doubled
                if current_place.ant != None and current_place.ant.doubled == False:
                    current_place.ant.damage *= 2
                    current_place.ant.doubled = True
                    #doubled.append(current_place.ant)

                #container
                if current_place.ant != None and current_place.ant.container == True: #add if ant in container is None
                    if current_place.ant.ant != None and current_place.ant.ant.doubled == False:
                        current_place.ant.ant.damage *= 2 
                        current_place.ant.ant.doubled = True

                            #doubled.append(current_place.ant.ant)
    ...
    def reduce_armor(self, amount):
        ...
        self.armor -= amount
        if self.armor <= 0:
            if self.realqueen == True:
                bees_win()
            elif self.realqueen == False:
                self.place.remove_insect(self)
...
def apply_effect(effect, bee, duration):
    ...
    def newaction(colony):

        nonlocal duration # So every time we call bee.action (newaction), we will automatically have the past duration???

        if duration > 0:
            effected(colony) # if this is not returned, at the last line below...
            duration -= 1
        else:
            original(colony) # after the duration ==0, will we keep going into this function although it's not useful any more???

    bee.action = newaction # ...this should be bee.action = None??? 
